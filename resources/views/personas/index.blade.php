...
@section('content')
<h1 class="mt-4 h3">Gestión de Personas</h1>
...
<div class="card mb-4">
    <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
        <div><i class="fas fa-user me-2"></i>Listado de personas</div>
        <a class="btn btn-sm btn-primary" href="{{ route('personas.create') }}">
            <i class="fas fa-plus me-1"></i>Nuevo
        </a>
    </div>
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-sm table-striped table-hover w-100" id="tabla-personas">
                <thead>
                    <tr>
                        <th>#</th>
                        <th>Persona</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">{{ $persona->fechaNacimientoFormateada() ?? '—' }}</td>
                        <td>{{ $persona->telefono ?? '—' }}</td>
                        <td class="text-center">
                            @if ( $persona->usuario )
                                <i class="fas fa-check-circle text-success" data-bs-toggle="tooltip" title="Tiene cuenta"></i>
                            @else
                                <i class="fas fa-minus-circle text-muted" data-bs-toggle="tooltip" title="Sin cuenta"></i>
                            @endif
                        </td>
                        <td class="text-end text-nowrap">
                            <a class="btn btn-sm btn-warning" data-bs-toggle="tooltip" title="Editar"
                               href="{{ route('personas.edit', $persona) }}"><i class="fas fa-edit"></i></a>
                            <form action="{{ route('personas.destroy', $persona) }}" method="POST" class="d-inline"
                                  data-confirm="¿Realmente desea eliminar esta persona?">
                                @csrf
                                @method('DELETE')
